...
def plot_one_box2(x, img, status, line_thickness=2):
    ...
    cv2.rectangle(img, c1, c2_label, color, -1, cv2.LINE_AA)  # Filled rectangle for label background
...
def plot_one_point(point, img, color=None, label="Point", point_radius=5):
    """
    在图像上绘制一个点和可选标签。
    ...
    # 绘制点
    point = (int(point[0]), int(point[1]))  # 确保点的坐标是整数
    cv2.circle(img, point, point_radius, color, thickness=-1, lineType=cv2.LINE_AA)

    # 绘制标签（如果有）
    if label:
        font_thickness = max(point_radius // 2, 1)  # 标签字体厚度
        font_scale = point_radius / 10  # 字体大小比例
        text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_thickness)[0]
        text_origin = (point[0] + 10, point[1] - 10)  # 标签位置（点的右上方）
        cv2.putText(img, label, text_origin, cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness=font_thickness, lineType=cv2.LINE_AA)

    # 返回点的信息
    point_info = f"Position: ({point[0]}, {point[1]}), Label: {label or 'None'}"
    return point_info
